...
open import Denot.LazyCoercionsOmniscient as LDC𝒪
...
module Denot.OmniLazyCoercions where

  open DenotCastStruct LDC𝒪.dcs using () renaming (⟦_⟧ to 𝒪⟦_⟧)
...
-}
  ...
  𝒪-typesound {Γ} {A} ($ f # P) ρ ρ∶Γ (⊢$ f P refl) = ℘-typing A P f
  ...
  𝒪-coerce-sound-⊇ (ƛ A ˙ N) V-ƛ ρ ρ∶Γ (⊢ƛ A ⊢N ⟨ refl , refl ⟩) (c ↣ d) {a} ν Λ-ν = 
    ⟨ ν , ⟨ Λ-ν , coerce-ok tt ⟩ ⟩
  𝒪-coerce-sound-⊇ (ƛ A ˙ N) V-ƛ ρ {Γ} {A ⇒ B} {A' ⇒ B'} ρ∶Γ (⊢ƛ A ⊢N ⟨ refl , refl ⟩) (c ↣ d) {a} (V ↦ w) 
    (Λ-↦ ⟨ u , ⟨ u∈'ΛN⟨c⟩' , coerce-fail q y z ⟩ ⟩ Vne) = 
    ⟨ V ↦ w , ⟨ Λ-↦ {!  !} Vne , coerce-ok (λ V∶A' → {!   !}) ⟩ ⟩ 
  𝒪-coerce-sound-⊇ (ƛ A ˙ N) V-ƛ ρ {Γ} {A ⇒ B} {A' ⇒ B'} ρ∶Γ (⊢ƛ A ⊢N ⟨ refl , refl ⟩) (c ↣ d) {a} (V ↦ w) 
    (Λ-↦ ⟨ .w , ⟨ w∈'ΛN⟨c⟩' , coerce-ok u∶B' ⟩ ⟩ Vne) = 
    ⟨ V ↦ w , ⟨ Λ-↦ {!  w∈'ΛN⟨c⟩' !} Vne , coerce-ok (λ V∶A' → {! 𝒪-typesound N (mem V • ρ) ? ⊢N w ? !}) ⟩ ⟩
  𝒪-coerce-sound-⊇ (ƛ A ˙ N) V-ƛ ρ ρ∶Γ (⊢ƛ A ⊢N ⟨ refl , refl ⟩) (⊥ ._ ⟨ ℓ ⟩ ._) {a} v v∈ = {! u↝v !}
  𝒪-coerce-sound-⊇ ($ f # P) V-const ρ ρ∶Γ (⊢$ f P refl) c {a} d d∈ = {!   !}
  𝒪-coerce-sound-⊇ (⟦ M , N ⟧) (V-pair vV vV₁) ρ ρ∶Γ (⊢cons ⊢M ⊢N refl) c {a} d d∈ = {!   !}
  𝒪-coerce-sound-⊇ (inl M other B) (V-inl vV) ρ ρ∶Γ (⊢inl B ⊢M refl) c {a} d d∈ = {!   !}
  𝒪-coerce-sound-⊇ (inr M other A) (V-inr vV) ρ ρ∶Γ (⊢inr  A ⊢M refl) c {a} d d∈ = {!   !}
  𝒪-coerce-sound-⊇ (M ⟨ c₁ ₍ .i ₎⟩) (V-wrap vV i) ρ ρ∶Γ (⊢wrap c₁ i ⊢M ⟨ refl , refl ⟩) c {a} d d∈ = {!   !}

{-
  𝒪-coerce-sound-⊆ : ∀ V → (vV : Value V) → ∀ ρ {Γ A B} 
                 → (ρ∶Γ : ∀ i d {A} → d ∈ ρ i → Γ ∋ i ⦂ A → ⟦ d ∶ A ⟧)
                 → (Γ⊢V∶A : Γ ⊢ V ⦂ A) → (c : Cast (A ⇒ B)) → {a : Active c}
                 → ∀ d → ¬ (isBlame d) → d ∈ 𝒞⟦ c ⟧ (𝒪⟦ V ⟧ ρ) → d ∈ 𝒪⟦ applyCast V Γ⊢V∶A vV c {a} ⟧ ρ
  𝒪-coerce-sound-⊆ (ƛ A ˙ N) V-ƛ ρ ρ∶Γ (⊢ƛ A ⊢N ⟨ refl , refl ⟩) (c ↣ d) {a} v ¬bv ⟨ u , ⟨ u∈ , u↝v ⟩ ⟩ = {! u∈ !}
  𝒪-coerce-sound-⊆ (ƛ A ˙ N) V-ƛ ρ ρ∶Γ (⊢ƛ A ⊢N ⟨ refl , refl ⟩) (⊥ ._ ⟨ ℓ ⟩ ._) {a} v ¬bv ⟨ u , ⟨ u∈ , u↝v ⟩ ⟩ = {! u↝v !}
  𝒪-coerce-sound-⊆ ($ f # P) V-const ρ ρ∶Γ (⊢$ f P refl) c {a} d ¬bd ⟨ u , ⟨ u∈ , u↝d ⟩ ⟩ = {!   !}
  𝒪-coerce-sound-⊆ (⟦ M , N ⟧) (V-pair vV vV₁) ρ ρ∶Γ (⊢cons ⊢M ⊢N refl) c {a} d ¬bd ⟨ u , ⟨ u∈ , u↝d ⟩ ⟩ = {!   !}
  𝒪-coerce-sound-⊆ (inl M other B) (V-inl vV) ρ ρ∶Γ (⊢inl B ⊢M refl) c {a} d ¬bd ⟨ u , ⟨ u∈ , u↝d ⟩ ⟩ = {!   !}
  𝒪-coerce-sound-⊆ (inr M other A) (V-inr vV) ρ ρ∶Γ (⊢inr  A ⊢M refl) c {a} d ¬bd ⟨ u , ⟨ u∈ , u↝d ⟩ ⟩ = {!   !}
  𝒪-coerce-sound-⊆ (M ⟨ c₁ ₍ .i ₎⟩) (V-wrap vV i) ρ ρ∶Γ (⊢wrap c₁ i ⊢M ⟨ refl , refl ⟩) c {a} d ¬bd ⟨ u , ⟨ u∈ , u↝d ⟩ ⟩ = {!   !}
-}
