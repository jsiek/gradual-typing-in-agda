  ...
  private
    wrapV-⊑-inv : ∀ {Γ Γ′ A A′} {V : Γ ⊢ A} {V′ : Γ′ ⊢ A′} {c : Cast (A ⇒ ⋆)}
      ...
      → Γ , Γ′ ⊢ V ⟪ i ⟫ ⊑ᶜ V′
        ------------------------
      → Γ , Γ′ ⊢ V ⊑ᶜ V′
    wrapV-⊑-inv v v' (I-inj g c) nd (⊑ᶜ-wrap (lpii-inj .g) lpVi imp) = contradiction refl nd
    ...
    ...       | ⊑ᶜ-wrap (lpii-inj _) _ imp = contradiction lp (nd⋢⋆ nd)
    ...
    dyn-value-⊑-wrap : ∀ {A′ B′} {V : ∅ ⊢ ⋆} {V′ : ∅ ⊢ A′} {c′ : Cast (A′ ⇒ B′)}
      ...
      → ∅ , ∅ ⊢ V ⊑ᶜ V′
        -----------------------
      → ∅ , ∅ ⊢ V ⊑ᶜ V′ ⟪ i′ ⟫
    dyn-value-⊑-wrap v v′ (Inert.I-inj () (cast .⋆ .⋆ _ _)) (⊑ᶜ-wrap (lpii-inj g) lpV imp)
      ...
      [ cons W₁ W₂ ,
        [ V-pair w₁ w₂ ,
          [ -- _ —→⟨ ξ {F = F-×₂ _} (ξ {F = F-cast _} (β-fst v w)) ⟩
            -- _ —→⟨ ξ {F = F-×₁ _ _} (ξ {F = F-cast _} (β-snd v w)) ⟩
            _ —→⟨ ξ {F = F-×₂ _} (cast v {a₁}) ⟩
            (↠-trans (plug-cong (F-×₂ _) rd*₁)
