      ...
      case ActiveOrInert c of λ where
        (inj₁ a) →
          case applyCast-catchup a (preserve-mult ⊢M rd*₁) ⊢M′ v v′ A⊑A′ B⊑A′ V⊑ of λ where
            ⟨ W , ⟨ w , ⟨ rd*₂ , W⊑ ⟩ ⟩ ⟩ →
              let ⊢V = preserve-mult ⊢M rd*₁ in
              ⟨ W , ⟨ w ,
                ⟨ ↠-trans (plug-cong (F-cast c) ⊢M rd*₁) (_ —→⟨ cast ⊢V v ⟩ rd*₂) ,
                  W⊑ ⟩ ⟩ ⟩
        (inj₂ i) →
          ⟨ V ⟨ c ₍ i ₎⟩ , ⟨ V-wrap v i ,
...
sim-fun-cast {W = W} (⊢wrap c i ⊢V 𝐶⊢-wrap) ⊢W ⊢V′ ⊢W′ (V-wrap v i) w v′ w′ i′ x′
  ...
  case Inert-Cross⇒ c i of λ where
    ⟨ x , ⟨ A₁ , ⟨ A₂ , refl ⟩ ⟩ ⟩ →
      case ⟨ uniqueness ⊢V′c′ (⊢wrap _ i′ ⊢V′ 𝐶⊢-wrap) , ⟨ A⊑A′ , B⊑A′ ⟩ ⟩ of λ where
        ⟨ refl , ⟨ fun⊑ A₁⊑C′ A₂⊑D′ , fun⊑ B₁⊑C′ B₂⊑D′ ⟩ ⟩ →
          let ⊢Wdomc = ⊢cast (dom c x) ⊢W 𝐶⊢-cast in
  ...
  case Inert-Cross× c i of λ where
    ⟨ x , ⟨ A₁ , ⟨ A₂ , refl ⟩ ⟩ ⟩ →
      case sim-β-fst ⊢V ⊢V′ ⊢W′ v v′ w′ V⊑V′W′ of λ where
        ⟨ M , ⟨ fstV↠M , M⊑V′ ⟩ ⟩ →
          case ⟨ uniqueness ⊢V′ ⊢V′† , ⟨ uniqueness ⊢W′ ⊢W′† , ⟨ A⊑A′ , B⊑A′ ⟩ ⟩ ⟩ of λ where
            ⟨ refl , ⟨ refl , ⟨ pair⊑ A₁⊑A₁′ A₂⊑A₂′ , pair⊑ B₁⊑B₁′ B₂⊑B₂′ ⟩ ⟩ ⟩ →
...
sim-case-cast {A} {A₁′} {A₂′} {B} {B₁′} {B₂′} {C} {C′} {V = V ⟨ c ₍ i ₎⟩} {V′} {M} {M′} {N} {N′}
              (⊢wrap c i ⊢V 𝐶⊢-wrap) ⊢V′ ⊢M ⊢M′ ⊢N ⊢N′ (V-wrap v i) v′ i′ x′
              (⊑-wrap A⊑A′ B⊑B′ V⊑V′ imp) M⊑M′ N⊑N′ =
  case Inert-Cross⊎ c i of λ where
    ⟨ x , ⟨ A₁ , ⟨ B₁ , refl ⟩ ⟩ ⟩ →
      case ⟨ A⊑A′ , B⊑B′ ⟩ of λ where
...
sim-case-cast {A} {A₁′} {A₂′} {B} {B₁′} {B₂′} {C} {C′} {V = V ⟨ c ₍ i ₎⟩} {V′} {M} {M′} {N} {N′}
              (⊢wrap c i ⊢V 𝐶⊢-wrap) ⊢V′ ⊢M ⊢M′ ⊢N ⊢N′ (V-wrap v i) v′ i′ x′
              (⊑-wrapl A⊑A′ B⊑A′ (⊢wrap c′ i′ ⊢V′† 𝐶⊢-wrap) V⊑V′) M⊑M′ N⊑N′ =
  case Inert-Cross⊎ c i of λ where
    ⟨ x , ⟨ A₁ , ⟨ B₁ , refl ⟩ ⟩ ⟩ →
      case ⟨ A⊑A′ , B⊑A′ ⟩ of λ where
        ⟨ sum⊑ A₁⊑A₂′ B₁⊑B₂′ , sum⊑ A₂⊑A₂′ B₂⊑B₂′ ⟩ →
