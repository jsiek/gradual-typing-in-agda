...
module ParamCastReduction
  ...
  canonical⋆ : ∀ {Γ} → (M : Γ ⊢ ⋆) → (Value M) → Σ[ A ∈ Type ] Σ[ M' ∈ (Γ ⊢ A) ] Σ[ c ∈ (Cast (A ⇒ ⋆)) ] Inert c × (M ≡ (M' ⟨ c ⟩))
  canonical⋆ .($ _) (V-const {k = ()})  
  canonical⋆ .(_ ⟨ _ ⟩) (V-cast{Γ}{A}{B}{V}{c}{i} v) = ⟨ A , ⟨ V , ⟨ c , ⟨ i , refl ⟩ ⟩ ⟩ ⟩
    
{-
  canonical⊎ : ∀ {Γ A B} → (M : Γ ⊢ A `⊎ B) → (Value M) → (Σ[ M' ∈ (Γ ⊢ A) ] (M ≡ (inl M'))) ⊎ (Σ[ M' ∈ (Γ ⊢ B) ] (M ≡ (inr M')))
  canonical⊎ ($ ()) V-const
  canonical⊎ (inl M') (V-inl v) = inj₁ ⟨ M' , refl ⟩
  canonical⊎ (inr M') (V-inr v) = inj₂ ⟨ M' , refl ⟩
  canonical⊎ .(_ ⟨ _ ⟩) (V-cast v) = {!!}
-}
