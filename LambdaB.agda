...
module LambdaB where
  ...
  open import ParamCastAuxABT pcs public
  ...
  applyCast : ∀ {Γ A B} → (M : Term) → Γ ⊢ M ⦂ A → (Value M) → (c : Cast (A ⇒ B))
                        → {a : Active c} → Term
  {- V : ι ⇒ ι   —→   V -}
  applyCast M ⊢M v c {A-id c} = M
  {- V : A ⇒ ⋆   —→   V : A ⇒ G ⇒ ⋆ -}
  applyCast M ⊢M v (cast A ⋆ ℓ cn) {A-inj c a-ng a-nd}
      with ground A {a-nd}
  ... | ⟨ G , cns ⟩ = (M ⟨ cast A G ℓ (proj₂ cns) ⟩) ⟨ cast G ⋆ ℓ unk~R ⟩
  {- V : G ⇒p ⋆ ⇒q G  —→   V
     V : G ⇒p ⋆ ⇒q H  —→   blame q -}
  applyCast M ⊢M v (cast ⋆ B ℓ cn) {A-proj c b-nd}
      with ground? B
  ... | yes b-g
          with canonical⋆ ⊢M v
  ...     | ⟨ G , ⟨ V , ⟨ c' , ⟨ i , ⟨ _ , refl ⟩ ⟩ ⟩ ⟩ ⟩
  ...
  applyCast-wt : ∀ {Γ A B} {M : Term} {c : Cast (A ⇒ B)}
    ...
    → (v : Value M) → (a : Active c)
      --------------------------------
    → Γ ⊢ applyCast M ⊢M v c {a} ⦂ B
  applyCast-wt ⊢M v (A-id _) = ⊢M
  applyCast-wt ⊢M v (A-inj (cast A ⋆ ℓ c~) ng nd) = ⊢cast _ (⊢cast _ ⊢M 𝐶⊢-cast) 𝐶⊢-cast
  applyCast-wt ⊢M v (A-proj (cast ⋆ B ℓ c~) nd)
    with ground? B
  ... | no  _ = ⊢cast _ (⊢cast _ ⊢M 𝐶⊢-cast) 𝐶⊢-cast
  ... | yes b-g with canonical⋆ ⊢M v
  ...   | ⟨ G , ⟨ V , ⟨ c' , ⟨ i , ⟨ ⊢V , refl ⟩ ⟩ ⟩ ⟩ ⟩
    with gnd-eq? G B {inert-ground c' i} {b-g}
  ...     | yes refl = ⊢V
  ...     | no  _    = ⊢blame B ℓ 𝐶⊢-blame


  open import CastStructureABT

  cs : CastStruct
  cs = record { precast = pcs ; applyCast = applyCast ; applyCast-wt = applyCast-wt }
