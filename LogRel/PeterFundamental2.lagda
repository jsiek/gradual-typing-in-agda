...
\end{code}
\begin{code}[hide]
compatible-literal {Γ}{c}{ι} = (λ γ γ′ → LRᵥ⇒LRᵀ c⊑c) ,ₚ (λ γ γ′ → LRᵥ⇒LRᵀ c⊑c)
  where
  LRᵥcc : ∀{𝒫}{dir} → 𝒫 ⊢ᵒ letᵒ (μᵒ pre-LRᵀ⊎LRᵥ) (LRᵥ (base⊑{ι}) dir ($ c) ($ c))
  LRᵥcc{𝒫}{dir}
  ...
  c⊑c{𝒫}{dir} = foldᵒ pre-LRᵀ⊎LRᵥ (inj₁ (($ᵀ ι , $ᵀ ι , base⊑{ι}) , dir , $ c , $ c)) (LRᵥcc{𝒫}{dir})
\end{code}
...
\end{code}
\begin{code}[hide]
compatible-blame{Γ}{A}{M} ⊢M = (λ γ γ′ → LRᵀ-blame) ,ₚ (λ γ γ′ → LRᵀ-blame)
  where
  LRᵀ-blame : ∀{𝒫}{A}{A′}{A⊑A′ : A ⊑ A′}{M}{dir} → 𝒫 ⊢ᵒ dir ∣ M ⊑ᴸᴿᵀ blame ⦂ A⊑A′
  LRᵀ-blame {𝒫} {A} {A′} {A⊑A′} {M} {≼} =
      foldᵒ pre-LRᵀ⊎LRᵥ (inj₂ ((A , A′ , A⊑A′) , ≼ , M , blame))
...
compatible-lambda{Γ}{A}{B}{C}{D}{N}{N′}{c}{d} ⊨N⊑N′ =
  (λ γ γ′ → LRᵥ⇒LRᵀ ⊢λN⊑λN′) ,ₚ (λ γ γ′ → LRᵥ⇒LRᵀ ⊢λN⊑λN′)
 where ⊢λN⊑λN′ : ∀{dir}{γ}{γ′} → (Γ ∣ dir ⊨ γ ⊑ᴸᴿ γ′)
                 ⊢ᵒ (dir ∣ ⟪ γ ⟫ (ƛ N) ⊑ᴸᴿᵥ ⟪ γ′ ⟫ (ƛ N′) ⦂ fun⊑ c d)
       ⊢λN⊑λN′ {dir}{γ}{γ′} =
         foldᵒ pre-LRᵀ⊎LRᵥ (inj₁ ((A ⇒ B , C ⇒ D , fun⊑ c d) , dir , ⟪ γ ⟫ (ƛ N) , ⟪ γ′ ⟫ (ƛ N′)))
         (Λᵒ[ W ] Λᵒ[ W′ ] →ᵒI 
...
anti-reduction-≼-R-one : ∀{𝒫}{A}{A′}{A⊑A′ : A ⊑ A′}{M′}{N′}
  → 𝒫 ⊢ᵒ (∀ᵒ[ M ]  (≼ ∣ M ⊑ᴸᴿᵀ N′ ⦂ A⊑A′)  →ᵒ  (M′ ⟶ N′)ᵒ  →ᵒ  (≼ ∣ M ⊑ᴸᴿᵀ M′ ⦂ A⊑A′))
anti-reduction-≼-R-one{𝒫}{A}{A′}{A⊑A′}{M′}{N′} =
   lobᵒ
   (Λᵒ[ M ] (→ᵒI (→ᵒI
   (case3ᵒ (unfoldᵒ pre-LRᵀ⊎LRᵥ (inj₂ ((A , A′ , A⊑A′) , ≼ , M , N′)) (Sᵒ Zᵒ))
   (∃ᵒE Zᵒ λ N → foldᵒ pre-LRᵀ⊎LRᵥ (inj₂ ((A , A′ , A⊑A′) , ≼ , M , M′))
      (inj₁ᵒ (∃ᵒI N (proj₁ᵒ Zᵒ ,ᵒ
      let IH = (Sᵒ (Sᵒ (Sᵒ (Sᵒ Zᵒ)))) in
      let ▷N⊑M′ : _ ⊢ᵒ ▷ᵒ (≼ ∣ N ⊑ᴸᴿᵀ M′ ⦂ A⊑A′)
          ▷N⊑M′ = →ᵒE (▷→ (→ᵒE (▷→ (∀ᵒE (▷∀ IH) N)) (proj₂ᵒ Zᵒ))) (monoᵒ (Sᵒ (Sᵒ Zᵒ))) in
      ▷N⊑M′))))
   (pureᵒE (Sᵒ Zᵒ) λ M′→N′ → pureᵒE Zᵒ λ N′→blame →
     foldᵒ pre-LRᵀ⊎LRᵥ (inj₂ ((A , A′ , A⊑A′) , ≼ , M , M′))
...
compatible-app {Γ}{A}{A′}{B}{B′}{c}{d}{L}{L′}{M}{M′} ⊨L⊑L′ ⊨M⊑M′ =
 ...
 ⊢LM⊑LM′ {dir}{γ}{γ′} =
   foldᵒ pre-LRᵀ⊎LRᵥ (inj₂ ((B , B′ , d) , dir , ⟪ γ ⟫ (L · M) , ⟪ γ′ ⟫ (L′ · M′)))
   {!!}
\end{code}
